...
class TodayPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final todayJournal = journalForDay(journalEntries, now);
    final todayMoney = moneyForDay(moneyEntries, now);
    ...
    final legacyPhoto = cover == null
        ...
        : null;
    final quote = quotes.isEmpty ? null : quoteForDay(quotes, now);
    final happy = todayJournal
        .where((entry) => entry.mood == Mood.happy)
        .toList();
    final unhappy = todayJournal
        .where((entry) => entry.mood == Mood.unhappy)
        .toList();
    final income = todayMoney
        .where((entry) => entry.kind == MoneyKind.income)
        .fold(0, (sum, entry) => sum + entry.cents);
    final expense = todayMoney
            ...
            children: [
              ...
              if (loading)
                ...
                _SectionTitle(
                  ...
                  action: TextButton(
                    onPressed: onWrite,
                    child: const Text('去记下'),
                  ),
                ),
                _TodayMoodOverview(happy: happy, unhappy: unhappy),
                const SizedBox(height: 36),
                const _SectionTitle('今日收支'),
                _DailyMoney(income: income, expense: expense),
                const SizedBox(height: 24),
                _PraiseWall(
                  entries: praiseEntries,
                  day: now,
  ...
  Future<void> _deletePraise(BuildContext context, PraiseEntry entry) async {
    ...
    if (confirmed == true) await onDeletePraise(entry);
  }
}
          ...
          sliver: SliverList.list(
            children: [
              ...
              if (widget.loading)
                ...
                _QuietEmpty('这个月还没有${_kind.label}记录。')
              else
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (widget.loading)
                        ...
                        const SizedBox(height: 10),
                        if (journals.isEmpty && praises.isEmpty)
                          const _QuietEmpty('这一天还没有回忆。'),
    ...
    return Row(
      ...
      children: [
        ...
        Text(
          ...
          style: const TextStyle(color: _muted, fontSize: 12, height: 1.65),
        ),
      ],
    );
  }
...
class _MainPhoto extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final tooltip = photoPath == null ? '选择首页照片' : '替换首页照片';
    return Column(
      children: [
        SizedBox(
          height: 342,
                  ...
                  child: AnimatedSwitcher(
                    ...
                    child: photoPath == null
                        ...
                        : Image.file(
                            ...
                            cacheWidth: 1080,
                            errorBuilder: (_, _, _) =>
                                const ColoredBox(color: Color(0xFFEAEDE6)),
                          ),
                  ),
                ),
              ),
...
class _TodayMoodOverview extends StatelessWidget {
  const _TodayMoodOverview({required this.happy, required this.unhappy});

  final List<JournalEntry> happy;
  final List<JournalEntry> unhappy;
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Expanded(
            child: _TodayMoodItem(
              key: const Key('today-mood-happy'),
              color: const Color(0xFFD5794B),
              label: '开心',
              entries: happy,
            ),
          ),
          ...
          Expanded(
            child: _TodayMoodItem(
              key: const Key('today-mood-unhappy'),
              color: const Color(0xFFB6BDB4),
              label: '不开心',
              entries: unhappy,
            ),
          ),
...
class _TodayMoodItem extends StatelessWidget {
  const _TodayMoodItem({
    super.key,
    required this.color,
    required this.label,
    required this.entries,
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(
            entries.isEmpty ? '今天还没有。' : entries.first.text,
            maxLines: 1,
